...
func newTableFrames(nfClient *NFClients, query models.RunnableQuery) (data.Frames, error) {
	...
	case models.PipelineResourceType:
		return newPipelineTableFrames(nfClient, query)
	case models.VertexResourceType:
		return newVertexTableFrames(nfClient, query)
	case models.IsbsvcResourceType:
		return newIsbsvcTableFrames(nfClient, query)
	}
...
func newVertexTableFrames(nfClient *NFClients, query models.RunnableQuery) (data.Frames, error) {
	...
	if *query.Vertex == "*" {
		...
		vertex, err := nfClient.GetPipelineVertex(queryNamespace, queryPipeline, *query.Vertex)
		if err != nil {
			return nil, err
		}
